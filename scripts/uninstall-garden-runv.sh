...
echo "[1/7] Desativando site do Apache..."
...
echo "[2/7] Removendo conteudo publicado desta instalacao..."
rm -rf "$WEB_ROOT"

echo "[3/7] Removendo renovacao cron desta instalacao, se existir..."
rm -f /etc/cron.d/certbot-renew-runv-garden

if [[ "$REMOVE_CERTS" == "true" ]]; then
  echo "[4/7] Removendo certificados do dominio desta instalacao..."
  certbot delete --non-interactive --cert-name "$APP_DOMAIN" || true
else
  echo "[4/7] Certificados preservados. Use REMOVE_CERTS=true para apagar."
fi

if [[ "$REMOVE_CODE" == "true" ]]; then
  echo "[5/7] Removendo codigo-fonte local desta instalacao..."
  rm -rf "$APP_DIR"
else
  echo "[5/7] Codigo-fonte preservado em $APP_DIR. Use REMOVE_CODE=true para apagar."
fi

if [[ "$REMOVE_DATA" == "true" ]]; then
  echo "[6/7] Removendo dados persistentes desta instalacao..."
  rm -rf "$APP_DATA_DIR"
else
  echo "[6/7] Dados persistentes preservados em $APP_DATA_DIR. Use REMOVE_DATA=true para apagar."
fi
